...
def solve():
    # [10] 狀態標記：記錄當前的雙引號應該是左引號還是右引號
    # True 代表下一個 " 是開始 (``)，False 代表是結束 ('')
    is_opening = True
    
    # [10] 從 sys.stdin 讀取所有內容，包含換行
    # 對於這類需要保持格式的題目，一次讀完通常最簡單
    content = sys.stdin.read()
    
    # 準備一個清單來儲存處理後的字元，最後再 join (效能較佳)
    output = []
    ...
    # [8] 使用 join 將清單結合成完整字串並印出
    # 注意：sys.stdin.read() 已經保留了換行符號，所以用 print(..., end='')
...
# 一次性讀取所有測資
data = sys.stdin.read()
first = True # 開關：是否為左引號

# 逐字處理並直接輸出
for c in data:
    if c == '"':
        # 根據開關狀態決定輸出，並同時切換開關
        print("``" if first else "''", end="")
        first = not first
    else:
        print(c, end="")
```
